...
def load_data():
    X = []
    with open('ccss_timeseries_all_payers.json', 'r') as rfile:
        for line in json_lines.reader(rfile):
            X.append(line['ts_game_start_counts_last_30_days_buckets'] + line['ts_hc_inflow_count_last_30_days_buckets']
                     + line['ts_hc_outflow_count_last_30_days_buckets'] + line['ts_usd_spend_last_30_days_buckets']
                     + line['ts_win_game_counts_last_30_days_buckets'])
    X = np.array(X)
    # ln_n_plus data normalization
    X = np.log(X+1)
    # add noise to dataset
    noise = tf.random_normal(shape=X.shape, mean=0.0, stddev=1e-8, dtype=tf.float32)
    X_noise = X + noise
    print('shape of X_noise: ', X_noise.shape)
    return X, X_noise, len(X)


def config_init(dataset):
    if dataset == 'ao':
        return 5*30, 50, 4, 3, 'linear'
...
def resolve_convolutional_variational_autoencoder2(data_noise, latent_dim, trainable=True):
    input_shape = (tf.shape(data_noise)[0], 1, 30, 5)
    _input = tf.reshape(data_noise, input_shape)

    with tf.name_scope("convolution_encoder"):
        cnn1 = tf.layers.conv2d(_input, filters=16, kernel_size=(1, 7), strides=(1, 1), padding='valid', activation=tf.nn.elu)
        print('cnn1: ', cnn1)

        cnn2 = tf.layers.conv2d(cnn1, filters=32, kernel_size=(1, 5), strides=(1, 2), padding='same', activation=tf.nn.elu)
        print('cnn2: ', cnn2)

        cnn3 = tf.layers.conv2d(cnn2, filters=64, kernel_size=(1, 3), strides=(1, 2), padding='same', activation=tf.nn.elu)
        print('cnn3: ', cnn3)

        dim_before_flatten = tf.shape(cnn3)
        cae_encoder = tf.layers.Flatten()(cnn3)
        ...
        with tf.name_scope("convolution_decoder"):
            ...
            cae_decoder = tf.layers.Conv2DTranspose(filters=32, kernel_size=(1, 3), strides=(1, 2), padding='same', activation=tf.nn.elu)(cae_decoder)
            print('cae_decoder2: ', cae_decoder)

            cae_decoder = tf.layers.Conv2DTranspose(filters=16, kernel_size=(1, 5), strides=(1, 2), padding='same', activation=tf.nn.elu)(cae_decoder)
            print('cae_decoder3: ', cae_decoder)

            cae_decoder = tf.layers.Conv2DTranspose(filters=5, kernel_size=(1, 7), strides=(1, 1), padding='valid', activation=None)(cae_decoder)
            print('cae_decoder4: ', cae_decoder)
        output = tf.reshape(cae_decoder, shape=tf.shape(data_noise))
        print('output: ', output)

        return data_noise, z_mu, z_sigma, z, gamma, output


'''
as a reference of network architecture, shall not be used
'''
def resolve_convolutional_variational_autoencoder(data, latent_dim, trainable=True):
    input_shape = (tf.shape(data)[0], original_dim, 1)
    _input = tf.reshape(data, input_shape)

    with tf.name_scope("convolution_encoder"):
        cnn1 = tf.layers.conv1d(_input, filters=32, kernel_size=5, strides=1, padding='same', activation=tf.nn.elu)
        print('cnn1: ', cnn1)

        cnn2 = tf.layers.conv1d(cnn1, filters=32, kernel_size=5, strides=2, padding='valid', activation=tf.nn.elu)
        print('cnn2: ', cnn2)

        cnn3 = tf.layers.conv1d(cnn2, filters=64, kernel_size=5, strides=1, padding='same', activation=tf.nn.elu)
        print('cnn3: ', cnn3)

        cnn4 = tf.layers.conv1d(cnn3, filters=64, kernel_size=3, strides=2, padding='valid', activation=tf.nn.elu)
        print('cnn4: ', cnn4)

        cnn5 = tf.layers.conv1d(cnn4, filters=128, kernel_size=3, strides=2, padding='valid', activation=tf.nn.elu)
        print('cnn5: ', cnn5)

    dim_before_flatten = tf.shape(cnn5)
    cae_encoder = tf.layers.Flatten()(cnn5)
    print('cae_encoder: ', cae_encoder)

    # flatten_dim = tf.shape(cae_encoder)[0]
    ...
    with tf.name_scope("convolution_decoder"):
        cae_decoder = tf.layers.dense(z, 69*128, activation=tf.nn.elu)
        cae_decoder = tf.reshape(cae_decoder, shape=dim_before_flatten)
        print('cae_decoder1: ', cae_decoder)

        cae_decoder = tf.contrib.nn.conv1d_transpose(value=cae_decoder, filter=tf.get_variable(name='filter4', shape=[3, 64, 128], dtype=tf.float32), output_shape=tf.shape(cnn4), stride=2, padding='VALID')
        cae_decoder = tf.nn.elu(cae_decoder)
        print('cae_decoder2: ', cae_decoder)

        cae_decoder = tf.contrib.nn.conv1d_transpose(value=cae_decoder,
                                                     filter=tf.get_variable(name='filter3', shape=[3, 64, 64],
                                                                            dtype=tf.float32),
                                                     output_shape=tf.shape(cnn3), stride=2, padding='VALID')
        cae_decoder = tf.nn.elu(cae_decoder)
        print('cae_decoder3: ', cae_decoder)

        cae_decoder = tf.contrib.nn.conv1d_transpose(value=cae_decoder,
                                                     filter=tf.get_variable(name='filter2', shape=[5, 32, 64],
                                                                            dtype=tf.float32),
                                                     output_shape=tf.shape(cnn2), stride=1, padding='SAME')
        cae_decoder = tf.nn.elu(cae_decoder)
        print('cae_decoder4: ', cae_decoder)

        cae_decoder = tf.contrib.nn.conv1d_transpose(value=cae_decoder,
                                                     filter=tf.get_variable(name='filter1', shape=[5, 32, 32],
                                                                            dtype=tf.float32),
                                                     output_shape=tf.shape(cnn1), stride=2, padding='VALID')
        cae_decoder = tf.nn.elu(cae_decoder)
        print('cae_decoder5: ', cae_decoder)

        cae_decoder = tf.contrib.nn.conv1d_transpose(value=cae_decoder, filter=tf.get_variable(name='filter0', shape=[5, 1, 32], dtype=tf.float32), output_shape=tf.shape(_input), stride=1, padding='SAME')
        # cae_decoder = tf.nn.elu(cae_decoder)
        print('cae_decoder6:', cae_decoder)
...
data_noise = tf.placeholder(tf.float32, shape=(None, original_dim), name='data_noise')

theta_p, u_p, lambda_p = gmmpara_init()
_, z_mean, z_log_var, z, tempGamma, x_decoded_mean = resolve_convolutional_variational_autoencoder2(data_noise, latent_dim)

loss, recon_loss, latent_loss, latent_loss_scalar, recon_loss_scalar, loss_scalar = vae_loss(data, x_decoded_mean)

global_step = tf.Variable(0, trainable=False)
learning_rate = tf.math.maximum(tf.train.exponential_decay(0.002, global_step, 4500, 0.9), 0.000002)
...
with tf.Session() as sess:
    ...
    Input_noise = sess.run(X_noise)
    print('X: ', X[0:1])
    print('Input_noise: ', Input_noise[0:1])
    for i in range(epoch):
        ...
        for j in range(n_batches):
            inp_noise = Input_noise[aidx[ptr:ptr + training_batch_size], :]
            ...
            _, _ce, _rec_loss, _lat_loss, _lr, summary = sess.run([optimizer, loss, recon_loss, latent_loss,learning_rate, merged_training_summary],
                                                                  feed_dict={data: inp, data_noise: inp_noise, batch_size: training_batch_size})
    ...
    if predict:
        ...
        latent, cluster_prec_temp = sess.run([z, tempGamma], feed_dict={data: X, data_noise: X, batch_size: n_train})
        ...
        with open(predictFileName, 'w') as f_write:
            with open('ccss_timeseries_all_payers.json', 'r') as f_read:
                i = 0
                for line in json_lines.reader(f_read):
                    ...
                    data['ts_hc_inflow_count_last_30_days_buckets'] = line['ts_hc_inflow_count_last_30_days_buckets']
                    data['ts_hc_outflow_count_last_30_days_buckets'] = line['ts_hc_outflow_count_last_30_days_buckets']
                    data['ts_usd_spend_last_30_days_buckets'] = line['ts_usd_spend_last_30_days_buckets']
                    data['ts_win_game_counts_last_30_days_buckets'] = line['ts_win_game_counts_last_30_days_buckets']
                    data['code'] = latent[i]
