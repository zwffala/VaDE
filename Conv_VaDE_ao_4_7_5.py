...
def resolve_convolutional_variational_autoencoder2(data_noise, latent_dim, trainable=True):
    input_shape = (tf.shape(data_noise)[0], 4, 7, 5)
    _input = tf.reshape(data_noise, input_shape)

    with tf.name_scope("convolution_encoder"):
        cnn1 = tf.layers.conv2d(_input, filters=16, kernel_size=(1, 1), strides=(1, 1), padding='valid', activation=tf.nn.elu)
        print('cnn1: ', cnn1)

        cnn2 = tf.layers.conv2d(cnn1, filters=32, kernel_size=(4, 1), strides=(1, 1), padding='valid', activation=tf.nn.elu)
        print('cnn2: ', cnn2)

        cnn3 = tf.layers.conv2d(cnn2, filters=64, kernel_size=(1, 7), strides=(1, 1), padding='valid', activation=tf.nn.elu)
        print('cnn3: ', cnn3)
        ...
        with tf.name_scope("convolution_decoder"):
            ...
            cae_decoder = tf.layers.Conv2DTranspose(filters=32, kernel_size=(1, 7), strides=(1, 1), padding='valid', activation=tf.nn.elu)(cae_decoder)
            print('cae_decoder2: ', cae_decoder)

            cae_decoder = tf.layers.Conv2DTranspose(filters=16, kernel_size=(4, 1), strides=(1, 1), padding='valid', activation=tf.nn.elu)(cae_decoder)
            print('cae_decoder3: ', cae_decoder)

            cae_decoder = tf.layers.Conv2DTranspose(filters=5, kernel_size=(1, 1), strides=(1, 1), padding='valid', activation=None)(cae_decoder)
...
learning_rate = tf.math.maximum(tf.train.exponential_decay(0.002, global_step, 1000, 0.9), 0.000002)
