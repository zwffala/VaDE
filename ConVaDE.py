...
def gmmCollectingMeansAndCovariances(sample):
    ...
    g.fit(sample)
    means.append(g.means_.T)
    covariances.append(g.covariances_.T)
    ...
    for i in range(epoch):
        ...
        if i == gmmInitialEndEpoch:
            ...
            for _, item in enumerate(means):
                meanCurrent = item
                meanSum = np.add(meanPrev, meanCurrent)
                meanPrev = meanSum
            up_assign_op = u_p.assign(meanPrev / len(means))
            covariancePrev = np.zeros(shape=covariances[0].shape)
            for _, item in enumerate(covariances):
                covarianceCurrent = item
                covarianceSum = np.add(covariancePrev, covarianceCurrent)
                covariancePrev = covarianceSum
            lambda_assign_op = lambda_p.assign(covariancePrev / len(covariances))
            sess.run([up_assign_op, lambda_assign_op])
            print('up after gmm boostring: ', u_p.eval(sess))
            print('lambda_p after gmm boostring: ', lambda_p.eval(sess))
        for j in range(n_batches):
            inp = X[aidx[ptr:ptr + training_batch_size], :]
            ptr += training_batch_size
            if gmmInitialStartEpoch <= i < gmmInitialEndEpoch:
                sample = sess.run(z_mean, feed_dict={data: inp, batch_size: training_batch_size})
                gmmCollectingMeansAndCovariances(sample)
                continue
