...
def resolve_convolutional_variational_autoencoder2(data_noise, latent_dim, trainable=True):
    input_shape = (tf.shape(data_noise)[0], 4, 7, 1)
    _input = tf.reshape(data_noise, input_shape)

    with tf.name_scope("convolution_encoder"):
        cnn1 = tf.layers.conv2d(_input, filters=16, kernel_size=(4, 1), strides=(1, 1), padding='valid', activation=tf.nn.elu)
        print('cnn1: ', cnn1)

        cnn2 = tf.layers.conv2d(cnn1, filters=32, kernel_size=(1, 3), strides=(1, 2), padding='valid', activation=tf.nn.elu)
        print('cnn2: ', cnn2)
        ...
        with tf.name_scope("convolution_decoder"):
            ...
            cae_decoder = tf.layers.Conv2DTranspose(filters=16, kernel_size=(1, 3), strides=(1, 2), padding='valid', activation=tf.nn.elu)(cae_decoder)
