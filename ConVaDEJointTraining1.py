'''
First, train on recon loss and unit gaussian distribution
Second, Training data scope GMM param inital
Third, train on recon loss + latent loss
'''
...
def config_init(dataset):
    if dataset == 'mnist':
        return 784, 100, 10, 0.002, 0.002, 10, 0.9, 0.9, 1, 'sigmoid'
    if dataset == 'reuters10k':
...
def gmmpara_init():
    ...
    lambda_p = tf.get_variable(name='lambda', shape=[latent_dim, n_centroid], dtype=tf.float32,
...
def vade_loss(x, x_decoded_mean):
    ...
    if datatype == 'sigmoid':
        ...
        _recon_loss = alpha * original_dim * tf.losses.mean_squared_error(labels=x, predictions=x_decoded_mean)
